# Results

## Availability of specimens

### General - "holder claims"

- 68.93% taxa have type material available (256740 of 372445)
- 9.48% taxa (35297 of 372445) are represented by type material served from within their native range

### Specific - author cites

![Catalog number trends](catalognumbertrend.png)


## Availability of literature

### Open access

![Open access trend](oatrend.png)

![Open access trend (pc)](oatrendpc.png)

### Open access status

![Open access status trend](oastatustrend.png)

![Open access status trend (pc)](oastatustrendpc.png)

### Open access by taxon distribution

![Open access TDWG WGSRPD level 1](oatrend-dist-1.png)

![Open access TDWG WGSRPD level 1 (pc)](oatrend-dist-1-pc.png)

![Open access TDWG WGSRPD level 1 - tax novs only](oatrend-dist-1-taxnov.png)

![Open access TDWG WGSRPD level 1 - tax novs only (pc)](oatrend-dist-1-taxnov-pc.png)

![Open access TDWG WGSRPD level 2](oatrend-dist-2.png)

![Open access TDWG WGSRPD level 2 (pc)](oatrend-dist-2-pc.png)

![Open access TDWG WGSRPD level 2 - tax novs only](oatrend-dist-2-taxnov.png)

![Open access TDWG WGSRPD level 2 - tax novs only (pc)](oatrend-dist-2-taxnov-pc.png)

![Open access TDWG WGSRPD level 3](oatrend-dist-3.png)

![Open access TDWG WGSRPD level 3 (pc)](oatrend-dist-3-pc.png)

![Open access TDWG WGSRPD level 3 - tax novs only](oatrend-dist-3-taxnov.png)

![Open access TDWG WGSRPD level 3 - tax novs only (pc)](oatrend-dist-3-taxnov-pc.png)

